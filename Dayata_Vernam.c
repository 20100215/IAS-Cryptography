/**
 *  CRYPTOGRAPHY SIMULATION: [5] Vernam Cipher
...
FILE* OTPFile;			//one-time pad (file)
char* buffer;			//area for character manipulation
char* OTP;				//one-time pad (for vernam)
...
do{
        ...
        //HEADER
        printf("\n---------------------------------------------------------------------");
        printf("\n-------------- CRYPTOGRAPHY SIMULATION (Vernam Cipher) --------------");
...
void getKey(){
	...
	if(mode==1){
		makeOTPFile();
		printf("\nGenerated OTP file - \"OTP.bin\"");
		printf("\nPlease keep this file for decryption.\n");
	}else{
		printf("\n=== STEP 3: ===");
		printf("\nEnter OTP File Name.");
		printf("\n\nINPUT OTP FILE NAME: ");
...
void makeOTPFile(){
	OTPFile = fopen("OTP.bin","wb");
	int x;
	char temp;
	srand(time(0));//initialize random num generator
	OTP = (char*)calloc(length,sizeof(char));
	for(x=0;x<length;x++){
		temp = rand()%128;
		OTP[x] = temp;
	}
	fwrite(OTP,sizeof(char),length,OTPFile);
	fclose(OTPFile);
}


void openOTPFile(){
	OTPFile = fopen("OTP.bin","rb");
	int x;
	OTP = (char*)calloc(length,sizeof(char));
	fread(OTP,sizeof(char),length,OTPFile);
	fclose(OTPFile);
}
